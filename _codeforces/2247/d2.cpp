/*
Problem link: https://codeforces.com/contest/2247/problem/D2
*/
...
check no crossing of max of prev block over min of next block
WAHOOOO
*/
...
void update(vector<aaa>& st, int ind, aaa val, int at, int atl, int atr) {       // st[at] describes [atl, atr] inclusive
    ...
    if (left.y > right.x) temp.z = (atr + 1 - atl) / 2; // power of 2 needed to permute entire cur range is half of size of total cur range
    st[at] = temp;
}
...
void solve() {
    ...
    while(q--) {
        int i, x; cin >> i >> x;
        update(st, i, aaa{x, x, 0}, 1, 0, newn - 1);
        cout << query(st, 0, newn - 1, 1, 0, newn - 1).z << endl;
    }
